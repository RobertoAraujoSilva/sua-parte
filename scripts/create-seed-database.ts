...
class SeedDatabaseCreator {
  ...
  async createSeedDatabase(): Promise<void> {
    try {
  ...
  private async createSchema(db: Database.Database): Promise<void> {
    // Enable foreign keys
    ...
    db.prepare('INSERT INTO migrations (version, description, applied_at) VALUES (?, ?, ?)').run(
      '002', 'Create indexes for performance', now
    );
  }

  private async loadStudentsFromExcel(): Promise<SeedStudent[]> {
    ...
    try {
      ...
      // Successfully loaded Excel data
      console.log(`📊 Successfully loaded ${excelData.length} students from Excel file`);
  ...
  private createSampleStudents(): SeedStudent[] {
    const now = new Date().toISOString();
  ...
  private async insertProfiles(db: Database.Database): Promise<void> {
    const now = new Date().toISOString();
  ...
  private async insertStudents(db: Database.Database, students: SeedStudent[]): Promise<void> {
    const stmt = db.prepare(`
  ...
  private async insertSamplePrograms(db: Database.Database): Promise<void> {
    const now = new Date().toISOString();
    ...
    for (const program of programs) {
      stmt.run(
        program.id,
        program.semana_inicio,
        program.semana_fim,
        program.material_estudo,
        program.congregacao_id,
        program.status,
        program.created_at,
        program.updated_at
      );
  ...
  private async insertSampleAssignments(db: Database.Database, students: SeedStudent[]): Promise<void> {
    const programs = db.prepare('SELECT id FROM programas ORDER BY semana_inicio').all() as { id: string }[];
    const now = new Date().toISOString();
    ...
    for (const assignment of assignments) {
      stmt.run(
        assignment.id,
        assignment.programa_id,
        assignment.estudante_id,
        assignment.ajudante_id,
        assignment.parte,
        assignment.tema,
        assignment.tempo_minutos,
  ...
  private async insertSampleMeetings(db: Database.Database): Promise<void> {
    const now = new Date().toISOString();
    const currentDate = new Date();
    ...
    for (const meeting of meetings) {
      stmt.run(
        ...
        meeting.start_time,
        meeting.end_time,
        meeting.circuit_overseer_name,
        meeting.service_talk_title,
        meeting.closing_song_number,
        meeting.congregacao_id,
        meeting.status,
        meeting.created_at,
        meeting.updated_at
      );
  ...
  private mapCargo(cargo: string): string {
    if (!cargo) return 'publicador_batizado';
    
    const cargoLower = cargo.toLowerCase();
    
    if (cargoLower.includes('ancião') || cargoLower.includes('anciao')) return 'anciao';
    if (cargoLower.includes('servo ministerial')) return 'servo_ministerial';
    if (cargoLower.includes('pioneiro')) return 'pioneiro_regular';
    if (cargoLower.includes('não batizado') || cargoLower.includes('nao batizado')) return 'publicador_nao_batizado';
    if (cargoLower.includes('estudante')) return 'estudante_novo';
    
    return 'publicador_batizado';
  }

  private mapGenero(genero: string): string {
